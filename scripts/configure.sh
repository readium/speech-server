...
# Provider-specific install config (e.g. PocketTTS) lives in its own env file —
# see pocket-tts.env.
...
# ══════════════════════════════════════════════════════════════════════════════
# Provider picker (ready to extend once a second provider exists)
# ══════════════════════════════════════════════════════════════════════════════

do_manage_provider() {
    hdr "Providers"
    local provider
    provider=$(gum choose "pocket" --header "Provider") || true
...
do_show_config() {
    ...
    printf '  Config files: %s, %s\n\n' "$ENV_FILE" "$POCKET_ENV_FILE"
}

do_reset() {
    hdr "Reset"
    warn "This will delete .env / pocket-tts.env and stop the server."
    CHOICE=$(gum choose \
